...
if (vecinos = 4) [
  ...
  ask patches with [ pcolor = red ] [ ;; ask the burning trees
    if (random-float 1 < prob) [ask neighbors4 with [pcolor = green] [ ;; ask their non-burning neighbor trees
...
if (vecinos = 8) [
  ...
  ;; ask the burning trees to set fire to any neighboring non-burning trees
  ask patches with [ pcolor = red ] [ ;; ask the burning trees
    if (random-float 1 < prob) [ask neighbors with [pcolor = green] [ ;; ask their non-burning neighbor trees
      set pcolor red ;; to catch on fire
    ]]
    set pcolor red - 3.5 ;; once the tree is burned, darken its color
  ]
...
@#$#@#$#@
<experiments>
  <experiment name="experimento 1" repetitions="15" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>(count patches with [shade-of? pcolor red]) / initial-trees   * 100</metric>
    <steppedValueSet variable="density" first="40" step="1" last="80"/>
  </experiment>
  <experiment name="experimento 2" repetitions="5" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>(count patches with [shade-of? pcolor red]) / initial-trees   * 100</metric>
    <steppedValueSet variable="density" first="50" step="1" last="80"/>
    <steppedValueSet variable="prob" first="0.65" step="0.01" last="1"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@

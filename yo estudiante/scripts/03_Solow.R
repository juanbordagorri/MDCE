...
#     labels=c("k^alpha","s*k^alpha","(delta+n)*k","inversión","consumo"),cex=1.5)
      labels=c("f(k)","s*f(k)","(delta+n)*k","inversión","consumo"),cex=1.5)

k_star=(s/(delta+n))^(1/(1-alpha))
y_star=(s/(delta+n))^(alpha/(1-alpha))

# evolución de k 
plot(k,type="l",main="Evolución de k",xlab="t")
segments(0,k_star,200,k_star,col="red",lty=3)

# evolución de y
plot(y,type="l",main="Evolución de y",xlab="t")
segments(0,y_star,200,y_star,col="red",lty=3)

# evolución de la acumulación de capital
plot(s*k^alpha-k*(delta+n),ylab="s*f(k)-k*(delta+n)",
     main="Acumulación de capital per cápita",xlab="t")
...
out <- ode(y = y_ini, times = times, func = derivs,
           parms = NULL)
#out
plot(out[,1],out[,2],type="l",xlab="t",ylab="k",main="Evolución de k")
